...
var load = module.exports.load = function() {
	...
	sources.forEach(function(source){
		...
		if (source.url) {
			...
			fs.readFile(source.file, {encoding: "utf8"}, function(err, data){
				if (err) {
					console.log("Error occured when reading file of source '" + source.name + "': " + err);
				} else {
					var callback = function() {
			...
			console.log("Config.json contains invalid source. Source skipped.");
		}
	});
